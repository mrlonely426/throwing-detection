...
class WandbLogger():
    def __init__(self, opt, name, run_id, data_dict, job_type='Training'):
        ...
        '''
        print(self.wandb)#None
        print(self.wandb_run)#None
        print(self.data_dict)#{'train': './fall_data_test/images', 'val': './fall_data_test/images', 'test': './fall_data_valid_block/images', 'nc': 1, 'names': ['0']}
        exit()
        '''
        # It's more elegant to stick to 1 wandb.init call, but useful config data is overwritten in the WandbLogger's wandb.init call
        '''
        print(opt.resume)#False
        print(type(opt.resume))#<class 'bool'>
        print(isinstance(opt.resume, str))#False
        exit()
        '''
        '''
        if 和 elif 都没有运行
